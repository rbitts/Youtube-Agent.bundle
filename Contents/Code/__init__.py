  ...
  if movie:
    ...
    if guid and not (len(guid)>2 and guid[0:2] in ('PL', 'UU', 'FL', 'LP', 'RD')):
      try:
        ...
        thumb = HTTP.Request( poster )
        try: metadata.posters[poster] = Proxy.Preview(thumb)
        except: pass
      ...
      for e in sorted(media.seasons[s].episodes, key=natural_sort_key):
        ...
        for video in Dict(json_playlist_items, 'items') or {}:
          ...
          if videoId and videoId in filename:
            ...
            thumb_url                       = Dict(video, 'snippet', 'thumbnails', 'standard', 'url') or Dict(video, 'snippet', 'thumbnails', 'high', 'url') or Dict(video, 'snippet', 'thumbnails', 'medium', 'url') or Dict(video, 'snippet', 'thumbnails', 'default', 'url')
            ...
            Log.Info('[ ] thumbnail:    {}'.format(thumb_url))
            try: episode.thumbs[thumb_url] = Proxy.Preview(thumb)
            except: pass
            if first: 
              first = False
              try: metadata.posters[thumb_url] = Proxy.Preview(HTTP.Request(thumb_url))
              except: pass
          ...
          if result:
            ...
            try:
              ...
              thumb_url                       = Dict(video_details, 'snippet', 'thumbnails', 'standard', 'url') or Dict(video_details, 'snippet', 'thumbnails', 'high', 'url') or Dict(video_details, 'snippet', 'thumbnails', 'medium', 'url') or Dict(video_details, 'snippet', 'thumbnails', 'default', 'url')
              # picture                         = HTTP.Request(thumb).content
              ...
              Log.Info('[ ] thumbnail:    {}'.format(thumb_url))
              try: episode.thumbs[thumb_url] = Proxy.Preview(HTTP.Request(thumb_url))
              except: pass
              ...
              if first: 
                first = False
                Log.Info('[ ] posters: {}'.format(thumb_url))
                try: metadata.posters[thumb_url] = Proxy.Preview(HTTP.Request(thumb_url))
                except: pass
